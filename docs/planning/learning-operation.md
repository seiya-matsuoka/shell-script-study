...
学習対象・学習順序・各 Unit の内容そのものは `learning-curriculum.md` で管理し、本ドキュメントではリポジトリ構成、Unit ごとの成果物、Unit README の共通形式、サンプルコードの扱い、学習の進め方、実行環境、Git / GitHub 上での区切り、最終的なリポジトリ完成状態などの運用面を扱う。  
...
小見出しは原則として 1 file ごとには設けず、学習テーマやサブディレクトリなど、内容上のまとまりを単位として構成する。  
一つの小見出しの中で複数の Script / command を順番に実行し、それぞれの役割や確認内容を説明してよい。  
同じ file path を「対象」と「実行」の両方で繰り返すような冗長な記述は避け、実行 command を直接示した上で必要な説明を記載する。
...
`学習内容` の単純な要約や同じ説明の繰り返しではなく、実際に確認した挙動を踏まえた再整理、似た概念の比較、別の視点からの説明、後続 Unit や実務とのつながりなどを記載する。  
学習上重要な内容は `学習内容` と重複してもよいが、実践を経たことで理解を深められる記述にする。
...
### 9.3 自然なサンプルコードと実行過程の確認

サンプルコードは、教材として説明を表示するためだけの処理を大量に追加せず、単体で読んだ場合にも自然な Shell Script となることを優先する。  
教材を見やすくするためだけの見出しや説明文を `printf` などで大量に出力し、学習対象となる処理が見えにくくなる構成は避ける。  
一方で、Script 本来の処理として出力が必要な場合、値や exit status を観察する場合、signal handler など実際の挙動確認に必要な場合は `printf` などを通常どおり使用してよい。

実際に実行される command や展開後の値を確認することが学習に有効な場合は、必要に応じて `bash -x` を利用する。  
すべての Script を一律に `bash -x` で実行するのではなく、通常実行の方が適切な場合や trace に表示すべきでない値を扱う場合は使用しない。  
`bash -x` 自体の debugging 機能としての詳細は Unit 04 で扱う。

### 9.4 学習対象外の実装を作り込みすぎない
...
教材として説明したい内容は、学習対象の処理とは直接関係しない出力処理として大量に実装するのではなく、必要に応じてソースコードコメントや README に記載する。  
コメントを付けるべきか迷う場合は、学習時や後から読み返したときの理解に役立つ可能性があるなら、記載する側に寄せる。
